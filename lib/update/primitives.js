...
ReplaceInArray.prototype.validate = function(primitive){
    if(this.target === primitive.target && this.position === primitive.position) {
        throw new Error('Two or more ReplaceInArray primitives have the same target object and selector.');
    }
};
...
ReplaceInObject.prototype.validate = function(primitive){
    if(this.target === primitive.target && this.pair === primitive.pair) {
        throw new Error('Two or more ReplaceInObject primitives have the same target object and selector.');
    }
};
