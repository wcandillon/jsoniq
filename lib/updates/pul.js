...
PUL.prototype.apply = function(store){
    ...
    this.udps.replaceInObject.forEach(function(udp){
        udp.apply(store);
    });
    ...
    this.udps.renameInObject.forEach(function(udp){
        udp.apply(store);
    });
    //4. jupd:insert-into-object
    this.udps.insertIntoObject.forEach(function(udp){
        udp.apply(store);
    });
    ...
    var rightToLeft = function(a, b){ return b - a;};
    this.udps.replaceInArray.sort(rightToLeft).forEach(function(udp){
        udp.apply(store);
    });
    //6. jupd:delete-from-array
    this.udps.deleteFromArray.sort(rightToLeft).forEach(function(udp){
        udp.apply(store);
    });
    //7. jupd:insert-into-array
    this.udps.insertIntoArray.sort(rightToLeft).forEach(function(udp){
        udp.apply(store);
    });
};
...
PUL.prototype.normalize = function(){
    ...
    this.udps.deleteFromObject.forEach(function(udp){
        udp.keys.forEach(function(key){
            _.remove(that.udps.replaceInObject, { target: udp.target, key: key });
            _.remove(that.udps.renameInObject, { target: udp.target, key: key });
        });
    });
    return this;
};
...
PUL.prototype.parse = function(pul){
    ...
    pul.replaceInObject.forEach(function(udp){
        that.replaceInObject(udp.target, udp.key, udp.value);
    });
 ...
 */
PUL.prototype.replaceInObject = function(target, key, value){
    var newUdp = new ReplaceInObject(target, key, value);
    //The presence of multiple UPs of this type with the same (array,index) target raises an error.
    var udp = _.find(this.udps.replaceInObject, { target: target, key: key });
    if(udp) {
        throw new jerr.JNUP0009();
    } else {
